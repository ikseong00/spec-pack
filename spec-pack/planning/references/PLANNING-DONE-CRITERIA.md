...
- 각 `must_have` 항목에 `primary_actor`, `trigger_or_entry`, `visible_user_outcome`, `completion_signal`이 있다
- 각 핵심 기능에 relevant한 `preconditions_or_eligibility`, `blocked_or_not_allowed_cases`, `exception_or_recovery_notes`가 있다
- 핵심 사용자 flow와 first value moment가 정의되었다
- 필요한 데이터소스와 운영 의존성이 정리되었다
- design 또는 architecture handoff가 예상되면 `BUSINESS-OPS.md`의 owner freeze 핵심값이 정리되었다
...
## 2.25 Service Spec Completeness

planning done은 기능 목록이 아니라 서비스 명세가 닫힌 상태여야 한다.

- 모든 핵심 기능은 `누가`, `언제`, `무슨 조건에서`, `무엇을 하며`, `사용자에게 어떤 결과가 보여야 하는지`로 설명 가능하다
- 기능 조건은 허용/금지/예외/복귀 규칙까지 포함한다
- 정상 흐름만이 아니라 실패, 대기, 보류, 재시도, 만료, 재진입이 relevant하면 적혀 있다
- 역할 차이가 있으면 actor별 visible state와 action 차이가 정리되어 있다
- 외부 시스템/운영 개입이 product behavior를 바꾸면 그 사실과 영향이 적혀 있다
- 같은 기능이나 상태를 문서마다 다른 용어로 부르지 않는다
- `completion_signal`은 KPI만이 아니라 user-visible 또는 operator-visible 완료 상태로 적힌다
- 동일 개념에 복수 명칭 가능성이 있으면 `Terminology Index`가 비어 있지 않다
- 개발자는 구현 방식을 스스로 정할 수 있지만, 제품 동작 의미를 추측할 필요는 없다
...
- `UX-IA.md`의 주요 step은 actor, trigger, expected outcome, detour 또는 recovery를 설명해야 한다
...
- `SCREEN-SPECS.md`의 P0/P1 screen은 relevant한 condition/rule, blocked state, recovery entry, role-specific variant를 담아야 한다
- `PRD.md`의 각 `must_have`는 policy/rule refs 또는 explicit `none`을 가진다
- `EXECUTION-HANDOFF.md`는 non-functional notes, exact next input, update checklist, traceability를 담아야 한다
- `EXECUTION-HANDOFF.md`는 must-have별 coverage check를 담아야 한다
- `PLANNING-RECORD.md`는 최소 `domain_shape_snapshot`과 applicable한 `workflow_contract`를 담아야 한다
- owner-sensitive shape면 `BUSINESS-OPS.md`에 `owner_freeze_snapshot`이 있고, `source_of_truth_owner`, `approval_owner`, `exception_owner`, `policy_or_sla_owner`, `required_visible_state_owner` 또는 explicit `N/A`가 있다
- regulated booking/revisit shape면 `booking_authority`, `revisit_or_rebook_recovery_owner`, `revisit_or_rebook_recovery_rule`, `required_visible_booking_state`가 explicit하다
- delegate/callback shape면 `delegate_authorization_artifact_or_flag`, `callback_owner_or_sla`, `delegated_callback_boundary`, `callback_audience_rule`, `required_visible_callback_state`가 explicit하다
- review/appeal shape면 `review_source_of_truth`, `review_sla`, `appeal_or_recheck_owner`, `appeal_path_rule`, `required_visible_review_state`가 explicit하다
...
- 개발자가 API / component / module 설계를 스스로 정할 수는 있지만, 제품/행동 질문을 다시 할 필요는 없다
- 개발자가 각 must-have 기능의 조건, 상태, 예외, 역할 차이를 문서만 읽고 복원할 수 있다
...
## 4.5 Developer Handoff Boundary

planning done은 개발자 handoff를 위해 아래를 만족해야 한다.

- 개발자가 다시 제품 discovery 질문을 하지 않아도 된다
- 하지만 API endpoint, component decomposition, module/class 구조는 문서가 강제하지 않는다
- 즉 문서는 `decision-complete, implementation-open` 상태여야 한다
- handoff의 완성도는 기술 설계 밀도가 아니라 서비스 명세 completeness로 판정한다
...
- required visible state owner가 명확하다
- appeal or override owner가 필요한 경우 명확하다
...
- callback owner or SLA와 callback authority boundary가 필요한 경우 정리되었다
...
- booking authority 또는 equivalent owner가 정리되었다
- revisit or rebook recovery owner가 필요한 경우 정리되었다
- revisit or rebook recovery rule과 required visible booking state가 필요한 경우 정리되었다
...
- required visible booking state가 필요한 경우 정리되었다
- 위 항목이 `BUSINESS-OPS.md`, `SCREEN-SPECS.md`, `EXECUTION-HANDOFF.md`에 projection되었다
...
- required visible state owner가 정리되었다
- communication boundary, audit or event requirement, appeal/regrade/callback rule, timeout or SLA가 정리되었다
- callback owner or SLA와 delegated callback boundary가 필요한 경우 정리되었다
- callback audience rule과 required visible callback state가 필요한 경우 정리되었다
- 위 항목이 `UX-IA.md`, `BUSINESS-OPS.md`, `SCREEN-SPECS.md`, `EXECUTION-HANDOFF.md`에 projection되었다
...
- review source of truth, appeal or recheck owner, appeal path rule, required visible review state가 필요한 경우 정리되었다
- knowledge-grounded가 같이 active가 아니면 citation/source-policy를 자동 요구하지 않는다
- 위 항목이 `SCREEN-SPECS.md`, `BUSINESS-OPS.md`, `EXECUTION-HANDOFF.md`에 projection되었다

### delegated-callback-visibility

- delegated or split-visibility shape면 `actor_split_ref`가 있다
- delegate authorization artifact or flag가 필요한 경우 명확하다
- callback owner or SLA가 명확하다
- delegated callback boundary와 callback audience rule이 명확하다
- required visible callback state가 명확하다
- communication boundary가 `BUSINESS-OPS.md`, `SCREEN-SPECS.md`, `EXECUTION-HANDOFF.md`에 projection되었다
- 위 항목 중 하나라도 unresolved면 planning done으로 보지 않는다
...
- design/architecture로 넘기는데 `owner_freeze_snapshot`이 `partial` 또는 `block`인데도 downstream translation을 시작한다
- delegated callback, appeal owner, booking authority 같은 modifier-shaped owner가 implicit하게 남아 있다

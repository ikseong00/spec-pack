...
## 6. Shared Memory / Compact Contract

phase-closing skill과 loop-facing agent는 종료 전에 아래 memory snapshot을 남긴다.

- `route_snapshot`
- `locked_facts_and_owners`
...
- memory snapshot은 새 truth를 만들지 않고 이미 잠긴 사실만 압축한다
- `planning-synthesis`는 이 snapshot을 handoff packet처럼 남겨 다음 stage가 같은 discovery를 반복하지 않게 한다
- answered question을 다시 열 수 있는 경우는 explicit reopen rule이 발화했을 때뿐이다

## 7. Compliance Self-Check

phase-closing skill과 checker agent는 complete를 주장하기 전에 아래를 짧게 확인한다.

- answered / suppressed question을 불필요하게 다시 열지 않았는가
- unsupported claim을 decision으로 승격하지 않았는가
- blocker를 compression 뒤로 숨기지 않았는가
- `exact_next_input`이 stage consumer 기준으로 actionable한가
- 이번 revision이 measurable delta를 만들었는가

이 self-check는 장문 에세이가 아니라 pass/fail note 수준이면 충분하다.

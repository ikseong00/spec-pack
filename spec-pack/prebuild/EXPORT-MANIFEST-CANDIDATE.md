...
이 문서는 shipped `spec-pack`의 multi-source export schema 초안을 설명한다.

candidate manifest:

- [export-manifest.candidate.json](/Users/ikseong/Desktop/develop/vibecoding/bbang/spec-pack/prebuild/export-manifest.candidate.json)

## Why A New Schema Is Needed

`planning-pack`은 single-source manifest로 충분하다.

하지만 `spec-pack`은 아래를 함께 실어야 한다.

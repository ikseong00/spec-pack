...
이 문서는 shipped `product-spec` surface가 end-user package로 유지되기 위해 무엇이 준비되어야 하는지 정리한다.

## Goal

- unified `product-spec`의 export 범위를 고정한다
- internal validation surface와 shipping surface를 분리한다
- install-ready 직전 남은 debt를 명시한다

## Candidate Export Set

current `product-spec` package 본체에 들어갈 것:
...
- `product-spec`은 cross-stage handoff까지 함께 ship한다
...
- [EXPORT-MANIFEST-CANDIDATE.md](/Users/ikseong/Desktop/develop/vibecoding/make-product-spec/product-spec/prebuild/EXPORT-MANIFEST-CANDIDATE.md)

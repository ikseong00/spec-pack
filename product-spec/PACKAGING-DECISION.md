...
현재 shipped shared root는 `product-spec`이다.
...
현재 shipped `product-spec`은 아래 원칙을 따른다.
...
현재 shipped `product-spec`은 `layout만 host-aware`가 아니라 `content overlay도 host-aware`로 가는 것이 맞다.

...
17. phase memory snapshot을 남겨 다음 stage가 route, owner, block 상태를 다시 추측하지 않게 한다
18. compliance self-check로 re-ask, unsupported decision, hidden blocker 여부를 짧게 확인한다
19. ready가 아니면 `not_ready_because`를 명시한다
20. 다음 action과 go / no-go 신호를 추천한다
...
- exact handoff packet for design or architecture
...
- phase memory snapshot이 없어서 downstream이 same discovery를 반복하게 되면 `done`이 아니다
- compliance self-check 없이 blocker를 압축 뒤로 숨기면 `done`이 아니다
